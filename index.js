...
client.once("ready", async () => {
  console.log(`VoidMint online jako ${client.user.tag}`);
...
client.on("messageCreate", async message => {
  ...
  if (message.content === "!regulamin") {
    const embed = new EmbedBuilder()
      ...
      .setColor("#00ff88")
      .addFields(
        { name: "1️⃣ Płatności", value: "Zakup oznacza akceptację regulaminu.\nBrak cofania płatności." },
        { name: "2️⃣ Zwroty", value: "Brak zwrotów po realizacji.\nReklamacje tylko z dowodem zakupu." },
        { name: "3️⃣ Realizacja", value: "Czas realizacji do 24h.\nMożliwe opóźnienia przy dużej ilości zamówień." },
        { name: "4️⃣ Bezpieczeństwo", value: "Nie odpowiadamy za błędnie podany nick." },
        { name: "5️⃣ Zachowanie", value: "Zakaz spamu, wyzwisk i prób scamu." }
      );
...
`);
  ...
  if (message.content === "!ticket") {
    const embed = new EmbedBuilder()
      ...
      .setDescription("Wybierz kategorię z menu.")
      .setColor("Purple");
...
Jeśli wszystko było w porządku — kliknij ✅  
Jeśli masz zastrzeżenia — kliknij ❌
...
client.on("interactionCreate", async interaction => {
  ...
  if (interaction.isButton()) {
    ...
    return interaction.reply({ content: "Głos zapisany ✅", ephemeral: true });
  }
  ...
  if (interaction.isChatInputCommand() && interaction.commandName === "opinia") {
    ...
    const obsluga = new TextInputBuilder()
      .setCustomId("obsluga")
    ...
    modal.addComponents(
      ...
      new ActionRowBuilder().addComponents(obsluga),
  ...
  if (interaction.isModalSubmit() && interaction.customId === "opinia_modal") {
    ...
    const obsluga = interaction.fields.getTextInputValue("obsluga");
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        { name: "Obsługa", value: `${obsluga}/5 ⭐`, inline: true },
        { name: "Czas realizacji", value: `${czas}/5 ⭐`, inline: true }
      )
      .setColor("Gold");
